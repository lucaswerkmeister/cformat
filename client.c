...
int main(int argc, char *argv[]) {
  ...
  strncpy(addr.sun_path, "/run/cformat", sizeof(addr.sun_path) - 1);
  #else
  char *runtime_dir = getenv("XDG_RUNTIME_DIR");
  if (!runtime_dir) error(EXIT_FAILURE, 0, "XDG_RUNTIME_DIR not set");
  snprintf(addr.sun_path, sizeof(addr.sun_path) - 1, "%s/cformat", runtime_dir);
  #endif
  ...
  free(cwd);
  // standard input
  if (pipeMode || argc == 1) { // TODO remove implicit pipe mode
